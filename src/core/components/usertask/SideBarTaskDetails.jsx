...
import { UpdateSingleTask } from '../../../service/operations/taskApi.js';
import toast from 'react-hot-toast';
import { useDispatch, useSelector } from 'react-redux';
import { CreateCommentApi } from '../../../service/operations/commentApi.js';
import { getUserDetail } from '../../../customHooks/user.js';
import useTimeAgo from '../../../customHooks/useTimeAgo.js';
import CommentArea from './CommentArea.jsx';
import { addAssignmentComment, updateAssignmentDocument } from '../../../features/project/singleprojectSlice.js';


function SideBarTaskDetails({ data, assign_user_id }) {
    ...
    useEffect(() => {
        try {
            if (typeof data.documents === "string") {
                setImageDocument(JSON.parse(data.documents));
            } else {
                setImageDocument(data.documents || []);
            }
        } catch (error) {
            console.error("Error parsing documents:", error, "Raw data:", data.documents);
            setImageDocument([]);
        }
    }, [data.documents]);
    ...
    const handleSubmit = (e) => {
        ...
        if (file) {
            ...
            UpdateSingleTask(formData, data.id)
                .then((res) => {
                    console.log(res)
                    if (res.data && res.data.documents) {
                        const resdocument = res.data.documents;
                        dispatch(updateAssignmentDocument({ task_id: data.id, project_id: data.project_id, resdocument }))
                        setImageDocument(res.data.documents)
    ...
    const uploadeduser = users.filter((item) => item.id === data.req_user_id);
    ...
    const handleCommentSubmit = async () => {
        ...
        try {
            ...
            dispatch(addAssignmentComment({ task_id: data.id, project_id: data.project_id, newComment }));
    ...
    return (
        <div className="d-block"  >
            ...
            <div className='col-12 d-flex justify-content-between mt-2' >
                <div className=' d-flex justify-content-start gap-3' >
                    ...
                    <div className='d-flex' >
                        <span className='my-auto recent-project-card-timeleft' style={{ fontSize: "16px" }} >{data.task_heading}</span>
                    </div>
                </div>
                <div className=' d-flex justify-content-end gap-3 d-none' >
            ...
            <div className='col-12 d-flex gap-5 justify-content-start project-detail-list my-4' >
                <span className=' my-auto w-25' style={{ fontSize: "12px" }} >Assignor</span>
                <div className='d-flex gap-1 justify-content-start project-detail-list' >

                    <div className='team-card-image' style={{ height: "30px", width: "fit-content" }} >
                        {
                            uploadeduser[0].image !== null ? <div> <img className='me-3 ' src={uploadeduser[0].image} style={{ zIndex: "5", height: "36px", width: "36px", borderRadius: "50%" }} /><span className=' mt-0 text-capitalize' style={{ fontSize: "16px" }} >{uploadeduser[0].name}</span></div> : <span className=' my-auto text-capitalize' style={{ fontSize: "16px" }} >{uploadeduser[0].name}</span>
                        }
                    </div>
            ...
            <div className='col-12 d-flex gap-5 justify-content-start project-detail-list' >
                ...
                <div className='d-flex gap-1 justify-content-start project-detail-list' >
                    <div className='d-flex' >
                        ...
                        <span className='my-auto recent-project-card-timeleft' style={{ fontSize: "14px", color: "#D84F67" }} >{data.duedate}</span>
                    </div>
                </div>
            </div>
            ...
            <div className='col-12 d-flex gap-5 justify-content-start project-detail-list mt-1' >
                ...
                <span className=' my-auto' style={{ fontSize: "12px" }} >{data.project_code}</span>
            </div>
            ...
            <div className='col-12 d-flex gap-5 justify-content-start project-detail-list mt-2' >
                ...
                <span className=' my-auto' style={{ fontSize: "12px" }} >no dependencies</span>
            </div>

            <div className='col-12  project-detail-list mt-4 p-2' style={{ backgroundColor: "#F4F4F4" }} >
                <p className='text1 my-auto w-25 m-0 fs-6' style={{ fontSize: "12px" }} >Task Description</p>
                <ul className='project-detail-list d-flex flex-column justify-content-start gap-3 mt-3' style={{ fontSize: "14px" }} >
                    <div dangerouslySetInnerHTML={{ __html: data.task_description }} />
                </ul>
            </div>

            <div className='col-12 d-flex flex-wrap gap-5 row-gap-1 mt-3' style={{ height: "fit-content" }}>
                {imagedocument.map((element, index) => (
                    <div key={index}>{renderFile(element.url, element.name, element.user_id, users)}</div>
                ))}

                <div className='add-pdf my-auto' onClick={() => document.getElementById('singletaskmedia').click()} style={{ cursor: "pointer" }}>
            ...
            < div className="col-12 ">
                <div className='overview-detail-bottom-card mt-3' >
                    ...
                    <CommentArea comments={comments} task_id={data.id} project_id={data.project_id} userId={userId} assign_user_id={assign_user_id} />
                    ...
                    <div className='mt-3' >
                        <div className='mt-3 d-flex gap-2 justify-content-start' >
                            ...
                            <div className='w-100 position-relative' >
                                <textarea className='odbc-textarea  w-100' placeholder='Add a comment' onChange={(e) => handleCommentChange(e)} ></textarea>
                                <label className='justify-content-end          position-absolute odbc-textarea-bottom w-100 px-2' >
                                    <div className='d-flex justify-content-start gap-2 d-none' >
                                        <span className='odbc-iconarea d-flex' >
                                            <button className=' my-auto' >
                                    ...
                                    <div className='d-flex justify-content-end gap-2' >
                                        ...
                                        <button className='odbc-comment-btn' onClick={() => handleCommentSubmit()} >Comment</button>
                                    </div>
...
const renderFile = (url, name, user_id, users) => {
    const download = (e) => {
        fetch(url)
            ...
            .then(blob => {
                ...
                const fileExtension = url.split('.').pop().split('?')[0]; // Handles query params in URL
                const fileName = name ? name : `downloaded-file.${fileExtension}`;
    ...
    const uploadeduser = users.filter((item) => item.id === user_id);

    if (url.match(/\.(jpeg|jpg|gif|png)$/i)) {
        return (
            <div className='d-flex justify-content-start gap-3'>
                <div className='sidebarimage' >
                    <img src={url} className='border' alt="Uploaded file" style={{ width: '50px', height: '50px', objectFit: 'cover' }} />
                </div>
                <div className='d-flex flex-column justify-content-between' >
                    <div className='d-flex justify-content-between' >
                        <p className='m-0 recent-project-card-timeleft text-capitalize d-flex ' style={{ fontSize: "14px", width: "88px", overflow: "hidden" }}>{name.split(" ")[0]}</p>
                        {
                            name.split(" ")[0].length > 9 ? <span style={{ lineHeight: "15px", cursor: 'pointer' }} >...</span> : <span style={{ fontSize: "10px", cursor: 'pointer' }} ></span>
                        }
                        <i className="fa-solid fa-trash-can my-auto ms-4 text-danger" style={{ fontSize: "10px", cursor: 'pointer' }}  ></i>
                    </div>
                    <div>
                        <p className='m-0 project-detail-list fst-italic' style={{ fontSize: "10px" }}>uploaded by {uploadeduser[0].name}</p>

                    </div>

                    <div className='d-flex gap-3 ' >
                        <button className='border-0 bg-transparent recent-project-card-timeleft' style={{ fontSize: "12px" }} onClick={e => download(e)} >Download</button>
                        <a className='border-0 bg-transparent recent-project-card-timeleft text-decoration-none' style={{ fontSize: "12px" }} target='_blank' href={url} >View</a>
                    </div>
                </div>
            </div>
        );
        ...
        return (
            <div className='d-flex justify-content-start gap-3 '>
                <div>
                    <img src={IconPath[0]?.path} alt="PDF icon" style={{ backgroundColor: "#fff", border: "1px solid #fff" }} />
                </div>
                <div className='d-flex flex-column justify-content-between gap-1' >
                    <div>
                        <p className='m-0 recent-project-card-timeleft text-capitalize' style={{ fontSize: "14px" }}>{name.split(" ")[0]}</p>
                        <p className='m-0 project-detail-list' style={{ fontSize: "12px" }}>-{uploadeduser[0].name}</p>
                    </div>

                    <div className='d-flex gap-3 ' >
                        <button className='border-0 bg-transparent recent-project-card-timeleft' style={{ fontSize: "10px" }} onClick={e => download(e)} >Download</button>
                        <a className='border-0 bg-transparent recent-project-card-timeleft text-decoration-none' style={{ fontSize: "10px" }} target='_blank' href={url} >View</a>
                    </div>
                </div>
            </div>
        );
        ...
        return (
            <div className='d-flex justify-content-start gap-3'>
                <div>
                    <img src={IconPath[1]?.path} alt="File icon" style={{ backgroundColor: "#fff", border: "1px solid #fff" }} />
                </div>
                <div className='d-flex flex-column justify-content-between gap-1' >
                    <div>
                        <p className='m-0 recent-project-card-timeleft text-capitalize' style={{ fontSize: "14px" }}>{name.split(" ")[0]}</p>
